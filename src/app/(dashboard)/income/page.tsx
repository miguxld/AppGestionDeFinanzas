...
import { INCOME_CATEGORIES } from "@/shared/constants";
...
type IncomeFormValues = z.infer<typeof financialTransactionSchema>;
        ...
        <div className="space-y-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-[#8899cc] mt-1">Registra y rastrea tus fuentes de entrada de dinero.</p>
                </div>
            </div>
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        <DollarSign className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4a5880] group-focus-within:text-emerald-400 transition-colors" />
                                        <Input
                                            type="number"
                                            placeholder="0.00"
                                            className="pl-10 h-12 rounded-xl bg-white/5 border-white/8 text-white focus:ring-emerald-500/20"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        <FileText className="absolute left-3.5 top-3.5 w-4 h-4 text-[#4a5880] group-focus-within:text-emerald-400 transition-colors" />
                                        <Input
                                ...
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-[#4a5880] uppercase tracking-wider">Fecha Recibido</Label>
                                    <div className="relative group">
                                        <CalendarIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#4a5880] group-focus-within:text-emerald-400 transition-colors" />
                                        <Input
                                            type="date"
                                            className="pl-10 h-12 rounded-xl bg-white/5 border-white/8 text-white"
                                ...
                                <Button
                                    ...
                                    className="w-full h-12 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98]"
                                >
                                    {(isSubmitting || mutation.isPending) ? <Loader2 className="w-5 h-5 animate-spin" /> : "Guardar Ingreso"}
                                </Button>
                            </form>
                        </CardContent>
                    </Card>

                    <Card className="glass border-white/8 bg-gradient-to-br from-emerald-500/10 to-transparent">
                        <CardContent className="p-6">
                            <h4 className="text-sm font-bold text-white mb-2">Tip de Ahorro</h4>
                            <p className="text-xs text-[#8899cc] leading-relaxed">
                                Intenta automatizar el 10% de cada ingreso ordinario hacia tu fondo de emergencia apenas lo recibas.
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card className="glass border-white/8 overflow-hidden">
                        <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 h-20 px-6">
                            <CardTitle className="text-lg text-white">Historial Reciente</CardTitle>
                        </CardHeader>
                        <div className="divide-y divide-white/[0.03]">
                            {isLoadingHistory ? (
                                <div className="p-10 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-emerald-400" /></div>
                            ) : (history?.filter((t: any) => t.type === 'INCOME') || []).map((item: any, i: number) => (
                                <div key={i} className="p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <div className="flex items-center gap-6">
                                        <div className="text-right">
                                            <p className="text-lg font-black text-emerald-400 leading-none">+ {formatCurrencyGlobal(Number(item.amount), currency, exchangeRate)}</p>
                                            <p className="text-[10px] text-emerald-500/60 font-bold uppercase mt-1 tracking-tighter">
                                                {item.status}
                                            </p>
                                        </div>
                            ...
                            {history?.filter((t: any) => t.type === 'INCOME').length === 0 && !isLoadingHistory && (
                                <div className="p-10 text-center text-[#4a5880] text-sm italic py-20">No hay ingresos registrados</div>
                            )}
                        </div>
                    </Card>
                </div>
